...
test.each(testCases)(
  "parses @heliumblocks/rich-text-part config path %s",
  (path, expectedResult) => {
    expect(parseFocusedRichTextPartConfigPath(path)).toEqual(expectedResult);
  }
);

test("throws error on invalid @heliumblocks/rich-text-part config path", () => {
  expect(() => parseFocusedRichTextPartConfigPath("invalid path")).toThrowError(
    "Invalid @heliumblocks/rich-text-part config path"
  );
});
